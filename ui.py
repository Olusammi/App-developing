...
def get_input_file(label, key, default_filename):
    """
    Creates a file uploader, explicitly using st.sidebar.file_uploader.
    """
    # ***FIXED: Use st.sidebar.file_uploader to ensure widget stays in the sidebar***
    uploaded = st.sidebar.file_uploader(label, key=key) 
    
    if uploaded is not None:
        return uploaded
    else:
        # Use st.sidebar.info to keep status message in the sidebar context
...
def render_sidebar():
    """
    Renders the entire sidebar, using expanders for better space management.
    """
    st.sidebar.header("🛠️ Input & Settings")

    # --- 1. Emission Parameter Files (COLLAPSIBLE) ---
    with st.sidebar.expander("1. Emission Parameter Files (.csv)", expanded=False):
        st.info("Upload your COPERT-like parameter files or use defaults.")
    ...
    # --- 2. Traffic Data Uploads (COLLAPSIBLE) ---
    with st.sidebar.expander("2. Traffic and Link Data (.txt/.osm)", expanded=True): # Keep this section open by default
        link_osm = st.sidebar.file_uploader("Upload Link Data (OSM_ID, Length, Flow...)", key="link_data_key")
        osm_file = st.sidebar.file_uploader("Upload OSM Map File (.osm)", key="osm_map_key")

    st.sidebar.markdown("---")

    # --- 3. Calculation Metrics (COLLAPSIBLE) ---
    with st.sidebar.expander("3. Calculation Metrics & Methodology", expanded=True):
    ...
    # --- 4. Accuracy Settings (COLLAPSIBLE) ---
    with st.sidebar.expander("4. Accuracy Corrections", expanded=False):
    ...
    with st.sidebar.expander("5. Advanced Solver Settings", expanded=False):
